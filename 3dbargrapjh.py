...
sizes = [25] * len(x)  # Sizes for each point
...
# Bar parameters
bar_width = 0.5  # Width of each bar in the y-direction
bar_depth = 0.5  # Depth of each bar in the z-direction

# Create bars for each data point
for i in range(len(x)):
    ax.bar3d(x[i], y[i], 0, bar_width, bar_depth, z[i], alpha=0.8)

# Add labels for each bar
for i in range(len(x)):
    ax.text(x[i], y[i], z[i], labels[i], fontsize=5, ha='center')

# Add axis labels
ax.set_xlabel('Points Per Game (Height)')
ax.set_ylabel('Points Allowed (Width)')
ax.set_zlabel('Strength Of Schedule (Depth)')
